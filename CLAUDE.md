...
No PHP in the free plugin references Parsedown or SimpleXLSXGen — they are pro-only. The script still ships them because `sync-pro-to-free.sh` rewrites only `PLUGIN_SLUG`, so any divergence from the pro repo is clobbered on the next sync.
