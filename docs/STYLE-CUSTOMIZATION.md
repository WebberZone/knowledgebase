...
- __Vibrant__ - Contemporary design with bold typography, vibrant accents, and smooth interactions
...
Some styles may also include:

- ✅ __RTL Support__ - Right-to-left language support (Arabic, Hebrew, etc.)
- ✅ __Variants__ - Non-minified, minified, RTL, and minified RTL versions

### File Loading (Minified vs Non-Minified)

By default, the plugin will load the __minified__ stylesheet when `SCRIPT_DEBUG` is off.

If you add your own style file (for example, a custom `vibrant.css`), make sure the matching `vibrant.min.css` exists for production sites.

### How Styles Work

- __Free version__ shows only 2 styles (Classic and Vibrant)
...
:root {
    ...
    --wzkb-color-primary-dark: #1e40af;
    --wzkb-color-primary-light: #3b82f6;
    --wzkb-color-accent: #f59e0b;
    --wzkb-color-accent-dark: #d97706;
    --wzkb-color-text: #1f2937;
    --wzkb-color-text-medium: #6b7280;
    --wzkb-color-text-light: #9ca3af;
    --wzkb-color-white: #ffffff;
}
```
...
:root {
    ...
    --wzkb-color-primary-dark: #5b21b6;
    --wzkb-color-primary-light: #a78bfa;
    --wzkb-color-accent: #f59e0b;
    --wzkb-color-text: #111827;
}
```
...
For the Classic style "Product cards grid" layout, you can improve hover/focus contrast by overriding the product card text colors:
